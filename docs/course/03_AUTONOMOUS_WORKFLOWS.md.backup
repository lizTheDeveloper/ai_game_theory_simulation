...
The script follows a **structured workflow** with explicit stages:

```
Stage 1:  PRE-FLIGHT CHECKS (health, disk space, API availability)
Stage 2:  GIT OPERATIONS (fetch, checkout, clean)
Stage 3:  BRANCH CREATION (auto/worker-TIMESTAMP)
Stage 4:  ROADMAP ANALYSIS (read, parse, prioritize)
Stage 5:  RESEARCH REQUESTS (post to research channel)
Stage 6:  TASK SELECTION (pick highest priority ready task)
Stage 7:  IMPLEMENTATION (spawn orchestrator via Claude Code CLI)
Stage 8:  VALIDATION (run tests, check build)
Stage 9:  COMMIT & PUSH (create PR)
Stage 10: CLEANUP (restore branch, save metrics)
Stage 11: COMPLETE (log timing, exit status)
```

**Design principle:** Each stage is **idempotent and resumable**. If stage 5 fails, you can manually fix and jump to stage 6. The status file (`logs/autonomous/status_current.txt`) tracks current stage.
...
- Updates Claude Code to latest version
- Starts chatroom monitor if not running

**Why this matters:** Autonomous systems must **validate assumptions** before doing work. If disk is full, stop early rather than failing mid-implementation.
...
- Cleans untracked files (prevents stale code)

**Why this matters:** Each autonomous run starts from **clean state**. No leftover files from previous runs, no stale branches, just fresh main.
...
**What this does:**
- Creates timestamped branch (e.g., `auto/worker-20251107_140000`)
- Prefix `auto/` signals autonomous branch (merge orchestrator filters on this)

**Why this matters:** Every autonomous run gets a **unique branch**. Multiple workers can run in parallel (different machines) without conflicts.

---

#### Stage 4: Roadmap Analysis
...
**Why this matters:** Roadmap is the **source of truth**. The worker doesn't invent work - it executes what's in the roadmap.

---

#### Stage 5: Research Requests (STEP 0)

```bash
# Post research requests to research channel FIRST
# This allows research to run in parallel with implementation

claude-code chat --session-file .claude/research-worker-session "
Post to research channel: Scan roadmap for CRITICAL/HIGH items needing research.
Post specific research needs (parameters, sources, validation).
Format: [QUESTION] status with clear research ask.
"
```

**What this does:**
- Posts research requests to `research` channel
- Cynthia (research worker) monitors this channel
- Research happens in parallel with implementation (next stage)

**Why this matters:** **Research-first workflow**. Don't start implementing and then discover you need parameters. Post research needs early, let research happen in parallel.

---

#### Stage 6: Task Selection

```bash
# Priority order: CRITICAL > HIGH > MEDIUM
# Prefer tasks with research complete

TASK=$(select_highest_priority_ready_task "$CRITICAL_ITEMS" "$HIGH_ITEMS")

if [ -z "$TASK" ]; then
    log_warning "No ready tasks found in roadmap"
    exit 0  # Not an error - just nothing to do
fi

log_success "Selected task: $TASK"
```

**What this does:**
- Picks highest-priority task that's ready (research done, dependencies met)
- If no tasks ready, exits gracefully (not an error)

**Why this matters:** **Smart task selection**. Don't pick tasks that are blocked. Don't pick tasks without research. Pick what's actually implementable right now.

---

#### Stage 7: Implementation (The Big One)
...
**What this does:**
- Spawns Claude Code chat session (45-minute timeout)
- Uses orchestrator agent (coordinates specialists)
- Full workflow: research → implement → test → document
- Session persists across stages (can resume if interrupted)

**Why this matters:** This is where the **actual work happens**. The orchestrator spawns specialized agents (Roy for simulation code, Tessa for UI, etc.), coordinates their work, and produces a working implementation.

**Timeout justification:** 45 minutes allows for:
- Complex features (6-8 phases)
- Monte Carlo validation (N=10 runs, 5-10 minutes)
- Documentation updates
- But prevents runaway sessions (hard stop at 45m)
...
```bash
# Check if implementation successful
if git diff --quiet; then
    log_warning "No changes made - task may not be complete"
    exit 0
fi
...
if [ -f "package.json" ] && grep -q "\"test\":" package.json; then
    npm test || log_warning "Some tests failed (continuing anyway)"
fi
```

**What this does:**
- Verifies changes were made
- Runs TypeScript type checking (catches type errors)
- Runs tests if available (warnings only, not hard failure)

**Why this matters:** **Quality gate**. Don't push broken code. If TypeScript errors exist, fail the run. If tests fail, log warning but continue (tests might be flaky).
...
Autonomous implementation by worker at $TIMESTAMP.
...
gh pr create \\
  --title "feat: $TASK" \\
  --body "Autonomous implementation. Validation: Monte Carlo N=10 passed." \\
  --base main \\
  --head "$BRANCH_NAME"
```
...
**Why this matters:** **Proper git hygiene**. Every autonomous run creates a proper PR with clear title, description, and link to source branch.
...
**What this does:**
- Switches back to main (clean state for next run)
- Saves timing metrics to JSON
- Writes final status

**Why this matters:** **Observability**. Metrics enable monitoring: How long do runs take? Which tasks are slow? What's the success rate?
...
- Roadmap analysis: 30 seconds
- Research requests: 2-3 minutes
- Task selection: 30 seconds
- Implementation: 15-35 minutes (most time here)
- Validation: 2-5 minutes
- Commit & push: 1-2 minutes
- Cleanup: 30 seconds

**Hard timeout:** 45 minutes at Claude Code session level

**Why this timing:** Leaves 15-30 minute buffer before next :00 cycle. If run goes long, next run simply skips (no pile-up).
