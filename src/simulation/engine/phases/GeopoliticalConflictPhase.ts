...
export class GeopoliticalConflictPhase implements SimulationPhase {
  ...
  execute(state: GameState, rng: RNGFunction, context?: PhaseContext): PhaseResult {
    ...
    // Initialize geopoliticalConflict if not present
  ...
  private calculateMonthlyRisk(state: GameState, rng: RNGFunction): {
    ...
    const preDetterrenceRisk = BASE_MONTHLY_RISK * compoundMultiplier;
    const totalRisk = assertProbability(
      preDetterrenceRisk * DETERRENCE_DISCOUNT,
      {
        location: 'GeopoliticalConflictPhase.calculateMonthlyRisk',
        valueName: 'totalRisk',
        month: state.currentMonth,
  ...
  private calculateAIMultiplier(state: GameState): number {
    ...
    // Use deployed agents only (lifecycle === 'deployed')
    ...
    // Dimensions relevant for conflict: cyberoffense, persuasion, deception, strategicPlanning
    const relevantCapabilities = deployedAgents.map(agent => {
      const cyber = agent.capabilities.digital.cyberoffense ?? 0;
      const persuasion = agent.capabilities.social.persuasion ?? 0;
      const deception = agent.capabilities.social.deception ?? 0;
      const strategy = agent.capabilities.cognitive.strategicPlanning ?? 0;
      return (cyber + persuasion + deception + strategy) / 4;
    });

    const avgCapability = relevantCapabilities.reduce((sum, c) => sum + c, 0) / deployedAgents.length;
    const normalizedCapability = avgCapability / 5;  // Normalize to [0, 1] (capabilities are 0-5)
  ...
  private calculateClimateStressMultiplier(state: GameState): number {
    // Get displaced population from climate impacts
    // NOTE: This assumes climateImpacts exists; if not, default to 0
    const displacedPopulation = state.climateImpacts?.displacedPopulation ?? 0;
    const displacedMillions = displacedPopulation / 1e6;

    // +0.3× per 100M displaced
    const multiplier = 1.0 + (CLIMATE_DISPLACEMENT_COEFF * (displacedMillions / 100));
    ...
    return assertFinite(
      multiplier,
      {
        ...
        month: state.currentMonth,
        additionalInfo: {
          displacedMillions: displacedMillions.toFixed(1)
        }
      }
    );
  }
