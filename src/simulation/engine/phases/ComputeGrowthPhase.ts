...
export class ComputeGrowthPhase implements SimulationPhase {
  ...
  execute(state: GameState, rng: RNGFunction): PhaseResult {
    ...
    // Validate efficiency multipliers are reasonable (Moore's Law: ~2x every 2 years = 1.03x per month)
    assertInRange(state.computeInfrastructure.hardwareEfficiency, 1.0, 100.0, {
      location: 'ComputeGrowthPhase.execute (post-growth)',
      valueName: 'computeInfrastructure.hardwareEfficiency',
      month: state.currentMonth,
      additionalInfo: { note: 'Hardware efficiency accumulates over decades' },
    });
    ...
    assertInRange(state.computeInfrastructure.algorithmsEfficiency, 1.0, 100.0, {
      ...
      month: state.currentMonth,
      additionalInfo: { note: 'Algorithmic efficiency accumulates over decades' },
    });
