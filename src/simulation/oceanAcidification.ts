...
export function initializeOceanAcidificationSystem(rng?: () => number): OceanAcidificationSystem {
  ...
  return {
    ...
    pH: 7.9,                         // Current (2025): 7.9 ±0.05, down from 8.1 pre-industrial
    ...
    pHHistory: [7.9],               // Historical tracking (Month 0)
...
export function updateOceanAcidificationSystem(state: GameState, rng: () => number): void {
  ...
  const pH_DECLINE_RATE_PER_MONTH = {
    SSP1_1_9: -0.00001,   // -0.01 / 900 months (aggressive mitigation)
    SSP1_2_6: -0.00009,   // -0.08 / 900
    SSP2_4_5: -0.00019,   // -0.17 / 900 (moderate)
    SSP3_7_0: -0.00030,   // -0.27 / 900
    SSP5_8_5: -0.00043,   // -0.39 / 900 (business as usual)
  };
  ...
  // Research: Exponential decline (coralHealth/100)^1.5
  ...
  const baseFisheriesYield = Math.pow(oa.coralReefHealth / 100, 1.5);
