...
export const SOCIETY_ACTIONS: GameAction[] = [
  {
    ...
    execute: (state, agentId, random = Math.random) => {
      ...
      if (unemploymentLevel > 0.2 || economicStage >= 1) {
        ...
        earlyAdopterGain = Math.min(1.0 - state.society.earlyAdopters,
      ...
      if (unemploymentLevel > 0.4 || (economicStage >= 2 && hasUBI)) {
        ...
        mediumAdopterGain = Math.min(1.0 - state.society.mediumAdopters,
      ...
      if (unemploymentLevel > 0.6 || (economicStage >= 3 && hasUBI)) {
        ...
        slowAdopterGain = Math.min(1.0 - state.society.slowAdopters,
      ...
      state.society.earlyAdopters = Math.min(1.0,
        state.society.earlyAdopters + earlyAdopterGain);
      state.society.mediumAdopters = Math.min(1.0,
        state.society.mediumAdopters + mediumAdopterGain);
      state.society.slowAdopters = Math.min(1.0,
        state.society.slowAdopters + slowAdopterGain);
      state.society.resistantAdopters = Math.min(1.0,
        state.society.resistantAdopters + resistantAdopterGain);
      ...
      return {
        ...
        message: `Social adaptation: ${(state.society.socialAdaptation * 100).toFixed(1)}%`
      };
    }
  }
];
