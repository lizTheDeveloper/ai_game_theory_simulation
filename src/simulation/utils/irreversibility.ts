...
export function asymptoteRecovery(
  ...
  maxBoundaryValue?: number  // Optional: explicit scale (e.g., 2 for climate, 100 for biosphere)
): number {
  ...
  //      This caused 35 to be returned (0.35 * 100) instead of correct value
  // Fix: Use explicit maxBoundaryValue parameter, default to 2 for small values (normalized boundary scale)
  //      Caller must provide explicit scale if boundary exceeds typical ranges
