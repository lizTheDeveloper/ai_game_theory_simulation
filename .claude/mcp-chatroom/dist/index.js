...
function writeLastReadLine(channel, agent, line) {
    const file = getLastReadFile(channel, agent);
...
function getActiveAgents(channel) {
...
function addActiveAgent(channel, agent) {
    const file = getActiveFile(channel);
...
function removeActiveAgent(channel, agent) {
    const file = getActiveFile(channel);
    let agents = getActiveAgents(channel);
    agents = agents.filter(a => a !== agent);
    if (agents.length > 0) {
        ...
        switch (name) {
            case 'chatroom_post': {
                ...
                if (!fs.existsSync(channelFile)) {
                    fs.writeFileSync(channelFile, `# ${channel.charAt(0).toUpperCase() + channel.slice(1)} Channel\n\n---\n`);
                }
                // Append message
            ...
            case 'chatroom_enter': {
                const { channel, agent, message = 'Entered channel, now monitoring' } = args;
            ...
            case 'chatroom_leave': {
                const { channel, agent, reason = 'Leaving channel' } = args;
            ...
            case 'chatroom_create_channel': {
                const { channel, description } = args;
                const channelFile = getChannelFile(channel);
                if (fs.existsSync(channelFile)) {
                    return {
                        content: [
