...
// Run 24 months, logging workflowAdaptation each month
for (let month = 0; month < 24; month++) {
  engine.simulateMonth(state);
  ...
  // Check for NaN
  if (isNaN(wa)) {
    console.log(`\n❌ MONTH ${month + 1}: workflowAdaptation = NaN`);
  ...
  // Log every 6 months
  if ((month + 1) % 6 === 0) {
    console.log(`Month ${month + 1}: workflowAdaptation = ${(wa * 100).toFixed(2)}%, research = $${budget.toFixed(1)}B`);
  }
}
